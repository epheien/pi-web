...
test("moves visible messages with a resized chat viewport", async () => {
  const { getScrollTopForResizedViewport } = await loadSubject();

  const keyboardOpenTop = getScrollTopForResizedViewport(400, 600, 300, 2000);
  assert.equal(keyboardOpenTop, 700);
  assert.equal(getScrollTopForResizedViewport(keyboardOpenTop, 300, 600, 2000), 400);
});

test("clamps resized chat viewport positions to its scroll range", async () => {
  const { getScrollTopForResizedViewport } = await loadSubject();

  assert.equal(getScrollTopForResizedViewport(100, 600, 900, 2000), 0);
  assert.equal(getScrollTopForResizedViewport(1800, 600, 300, 2000), 1700);
  assert.equal(getScrollTopForResizedViewport(0, 600, 300, 200), 0);
});
